...
> 测试：**56 项单测全过 + 25 项端到端冒烟全过**（2026-09-09，含第三轮自查回归 + property-based 性质测试 + 惰性淘汰回归 + SSE 转换/透传）+ Groq 真实链路验证通过（2026-09-01）
...
| `sse.py` | 240 | **v1.6/v1.7**：SSE 流式三件套。`parse_sse_lines` 行流分帧；`AnthropicToChatStream` 事件→Chat chunk 逐块转换（chat 客户端 ← anthropic 上游）：text_delta 即时下发、工具参数缓冲到 content_block_stop（既定折损）、usage 流尾合并、`synthetic_response()` 复用 `assistant_from_upstream` 落库；`tee_lines` + `ChatStreamCollector` 同协议直通透传（字节不动、旁路汇总） |
...
| `smoke_e2e.py` | 220 | 端到端冒烟——真起 mock + 网关两个子进程，跑 7 组 25 项断言（直通/跨协议/多轮链/预热/非法路由/SSE 转换/**SSE 透传+501**），退出码 0/1 |
...
| `TestChatStreamCollector` | **v1.7 新增**（3 项）：chat 直通流式旁路收集（文本+usage 汇总、工具槽位拼接、脏数据容错） |

## 四、已验证记录

| 验证 | 结果 | 日期 |
| ---- | ---- | ---- |
| 56 项单测（`-W error::ResourceWarning` 下零警告，含第三轮自查回归 + property-based + 惰性淘汰 + SSE 转换/透传） | ✅ 全过 | 2026-09-09 |
| 25 项端到端冒烟（子进程级，`tools/smoke_e2e.py`，含 SSE 转换/透传逐块到达实测 ~31ms + 未实现方向 501） | ✅ 全过 | 2026-09-09 |
