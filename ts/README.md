...
> 功能对齐 Python 版 v1.7：**56 项离线单测 + 25 项端到端冒烟全过**（与 Python 版用例一一镜像）。
...
npm test               # 56 项离线单测（node:test + Node 22 类型擦除，无需编译）
...
| `tests/test_offline.py`（56 项） | `tests/offline.test.ts`（56 项） | 用例一一镜像 |
...
4. **未移植**（后续 PR 补）：`src/webui/`（弹网页仪表盘）、`tools/verify_cache.py`（验站脚本）、`experiments/`（真实 API 实验脚本——研究代码，跟随 Python 版即可）。
