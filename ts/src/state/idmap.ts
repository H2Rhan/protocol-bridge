/** 工具调用 ID 双向持久映射（v1.4，问题清单组4#3；TS 移植自 src/state/idmap.py）。
 *
 ...
 * - `incoming(session, extId, proto)`：把某协议的外部 ID 归一到 canonical。
...
export class ToolIdMap {
  ...
  incoming(sessionKey: string, extId: string | null, proto: string): string | null {
    if (!extId) return extId;
    const row = this.db.prepare(
      "SELECT canon FROM tool_id_map WHERE session_key=? AND proto=? AND ext_id=?")
      .get(sessionKey, proto, extId) as { canon: string } | undefined;
    if (row) return row.canon;
    this.db.prepare(
      "INSERT OR IGNORE INTO tool_id_map " +
      "(session_key, proto, ext_id, canon, origin_proto, created_at) " +
      "VALUES (?,?,?,?,?,?)")
      .run(sessionKey, proto, extId, extId, proto, Date.now() / 1000);
    return extId;
  }

  /** canonical → 目标协议外部形式（稳定、可持久、按目标协议前缀铸造）。 */
  outgoing(sessionKey: string, canon: string | null, proto: string): string | null {
    if (!canon) return canon;
    // 已铸造过 → 直接用（跨轮稳定，前缀不抖动）
    const row = this.db.prepare(
      "SELECT ext_id FROM tool_id_map WHERE session_key=? AND proto=? AND canon=?")
      .get(sessionKey, proto, canon) as { ext_id: string } | undefined;
    if (row) return row.ext_id;
    // 来源协议就是目标协议 → canonical 原样返回（同协议字节无损）
    const origin = this.db.prepare(
      "SELECT origin_proto FROM tool_id_map WHERE session_key=? AND canon=? LIMIT 1")
      .get(sessionKey, canon) as { origin_proto: string } | undefined;
    if (origin && origin.origin_proto === proto) return canon;
    // 铸造新 ID 并持久化
    const ext = (PROTO_PREFIX[proto] ?? "call_") + randomUUID().replaceAll("-", "").slice(0, 24);
    this.db.prepare(
      "INSERT OR IGNORE INTO tool_id_map " +
      "(session_key, proto, ext_id, canon, origin_proto, created_at) " +
      "VALUES (?,?,?,?,?,?)")
      .run(sessionKey, proto, ext, canon,
           origin ? origin.origin_proto : proto, Date.now() / 1000);
    return ext;
  }

  size(sessionKey: string): number {
    const row = this.db.prepare(
      "SELECT COUNT(*) AS n FROM tool_id_map WHERE session_key=?")
